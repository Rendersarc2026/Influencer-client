 ...
 */
export function validateNumericInput(input: string | number | null | undefined): NumericValidationResult {
  ...
  if (typeof input === 'number') {
    if (isNaN(input) || !isFinite(input) || input < 0) {
      return { raw: String(input), isValid: false, value: null, error: 'Must be a positive number' };
    }
